...
async function buscarWppCliente(id){const c=BASE.find(x=>String(x.id)===String(id));if(!c)return;const b=document.querySelector('.wpp-find[data-id="'+CSS.escape(String(id))+'"]');try{if(!parent.portalBuscarWhatsAppCliente)throw new Error('Busca ainda não disponível no Portal.');if(b){b.disabled=true;b.textContent='Buscando...'}await parent.portalBuscarWhatsAppCliente({cliente_id:c.id,nome:c.nome,documento:c.documento});let achou=false;for(let i=0;i<18;i++){await new Promise(r=>setTimeout(r,5000));achou=await atualizarWppEncontrados(id);if(achou)break}if(!achou){if(b){b.disabled=false;b.textContent='Buscar novamente'}alert('Não encontrei um telefone público confiável para este cliente. Você ainda pode cadastrar manualmente.')}}catch(e){if(b){b.disabled=false;b.textContent='Buscar WhatsApp'}alert('Não foi possível iniciar a busca: '+e.message)}}
async function atualizarWppEncontrados(alvo=''){try{const r=await fetch('clientes-enrichment/latest.json?t='+Date.now(),{cache:'no-store'});if(!r.ok)return false;const d=await r.json();let mudou=false,achou=false;for(const c of BASE){const x=d.clientes?.[String(c.id)];if(x?.telefone&&!c.telefone){c.telefone=x.telefone;c.telefoneFonte=x.fonte||'Google Maps';c.telefoneUrl=x.maps_url||'';mudou=true}if(String(c.id)===String(alvo)&&x?.telefone)achou=true}if(mudou){save();render()}return achou}catch{return false}}
