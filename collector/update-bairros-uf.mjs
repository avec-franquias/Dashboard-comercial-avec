import fs from 'node:fs/promises';
import path from 'node:path';

const ROOT=process.cwd();
const UF=String(process.env.BAIRROS_UF||'').trim().toUpperCase();
const CONCURRENCY=Math.max(2,Math.min(16,Number(process.env.BAIRROS_CONCURRENCY||10)));
if(!UF) throw new Error('BAIRROS_UF obrigatorio');

const JSON_FILE=path.join(ROOT,'geo','bairros-br.json');
const CSV_FILE=path.join(ROOT,'geo','bairros-br.csv');
const key=s=>String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().toLocaleLowerCase('pt-BR').replace(/\s+/g,' ');

async function fetchJson(url,opts={},timeout=9000){
  const c=new AbortController();
  const t=setTimeout(()=>c.abort(),timeout);
  try{
    const r=await fetch(url,{...opts,signal:c.signal});
    if(!r.ok) throw new Error('HTTP '+r.status);
    return await r.json();
  }finally{clearTimeout(t)}
}

async function bairrosCidade(nome,uf){
  const q=new URLSearchParams({city:nome,state:uf,country:'Brazil',format:'jsonv2',limit:'2',addressdetails:'1'});
  let nr;
  try{
    nr=await fetchJson('https://nominatim.openstreetmap.org/search?'+q.toString(),{
      headers:{'User-Agent':'PortalFranqueadoAVEC/1.0','Accept-Language':'pt-BR'}
    },7000);
  }catch{return []}
  const p=(nr||[]).find(x=>String(x?.address?.country_code||'').toLowerCase()==='br')||(nr||[])[0];
  if(!p)return[];
  const id=Number(p.osm_id);
  const areaId=p.osm_type==='relation'?3600000000+id:p.osm_type==='way'?2400000000+id:null;
  if(!areaId)return[];

  const over='[out:json][timeout:12];area('+areaId+')->.a;(nwr["place"~"^(suburb|neighbourhood|quarter|borough)$"](area.a);nwr["boundary"="administrative"]["admin_level"~"^(9|10|11)$"](area.a););out tags;';
  for(const ep of ['https://overpass.kumi.systems/api/interpreter','https://overpass-api.de/api/interpreter']){
    try{
      const oj=await fetchJson(ep,{
        method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','User-Agent':'PortalFranqueadoAVEC/1.0'},
        body:'data='+encodeURIComponent(over)
      },14000);
      const arr=[...new Set((oj.elements||[]).map(x=>String(x.tags?.name||'').trim()).filter(Boolean))]
        .sort((a,b)=>a.localeCompare(b,'pt-BR'));
      if(arr.length)return arr;
    }catch{}
  }
  return[];
}

async function pool(items,worker,limit){
  let next=0; const out=new Array(items.length);
  async function run(){
    while(true){
      const i=next++;
      if(i>=items.length)return;
      try{out[i]=await worker(items[i])}
      catch(e){out[i]={cidade:items[i],bairros:[]}}
    }
  }
  await Promise.all(Array.from({length:Math.min(limit,items.length)},run));
  return out;
}

const base=JSON.parse(await fs.readFile(JSON_FILE,'utf8'));
base[UF] ||= {};
const csv=await fs.readFile(CSV_FILE,'utf8');
const cidades=[];
const seen=new Set();
for(const line of csv.replace(/^\uFEFF/,'').split(/\r?\n/).slice(1)){
  if(!line.trim())continue;
  const cols=line.split(';');
  const uf=cols[0], cidade=cols[2];
  if(uf!==UF||!cidade)continue;
  const k=key(cidade);
  if(seen.has(k))continue;
  seen.add(k);
  if(!Array.isArray(base[UF][k])||!base[UF][k].length)cidades.push(cidade);
}

console.log(UF,'faltantes',cidades.length,'concorrencia',CONCURRENCY);
const resultados=await pool(cidades,async cidade=>({cidade,bairros:await bairrosCidade(cidade,UF)}),CONCURRENCY);
let ok=0;
for(const r of resultados){
  if(r.bairros.length){base[UF][key(r.cidade)]=r.bairros;ok++;}
}
const outDir=path.join(ROOT,'tmp-bairros');
await fs.mkdir(outDir,{recursive:true});
await fs.writeFile(path.join(outDir,UF+'.json'),JSON.stringify({uf:UF,cidades:base[UF]},null,0),'utf8');
console.log(UF,'preenchidos',ok,'total agora',Object.values(base[UF]).filter(x=>Array.isArray(x)&&x.length).length);
